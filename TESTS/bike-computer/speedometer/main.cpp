...
void check_current_speed(const std::chrono::milliseconds &pedalRotationTime,
                         uint8_t traySize, uint8_t gearSize,
                         float wheelCircumference, float currentSpeed) {
  ...
  float trayGearRatio =
      static_cast<float>(traySize) / static_cast<float>(gearSize);
  float distancePerPedalTurn = trayGearRatio * wheelCircumference;
  float expectedSpeed =
      (distancePerPedalTurn / 1000.0f) * pedalRotationsPerHour;
...
static control_t test_gear_size(const size_t call_count) {
  ...
  for (uint8_t gearSize = bike_computer::kMaxGearSize;
       gearSize <= bike_computer::kMaxGearSize; gearSize++) {
    // set the gear
    ...
    check_current_speed(pedalRotationTime, traySize, gearSize,
                        wheelCircumference, currentSpeed);
  }
...
static control_t test_distance(const size_t call_count) {
  ...
  for (uint8_t index = 0; index < nbrOfTravelTimes; index++) {
    ...
    check_distance(pedalRotationTime, traySize, gearSize, wheelCircumference,
  ...
  for (uint8_t index = 0; index < nbrOfTravelTimes; index++) {
    ...
    float distance = compute_distance(pedalRotationTime, traySize, gearSize,
                                      wheelCircumference, travelTimes[index]);
    expectedDistance += distance;
    ...
    printf("  Expected distance is %f, current distance is %f\n",
           expectedDistance, traveledDistance);
    TEST_ASSERT_FLOAT_WITHIN(kAllowedDistanceDelta, expectedDistance,
                             traveledDistance);
  }

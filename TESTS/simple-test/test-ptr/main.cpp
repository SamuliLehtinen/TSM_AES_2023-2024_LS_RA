...
/**
 * Test that multiple instances of unique pointers correctly manage the reference count
 * to release the object at the correct point
 */
void test_uniquePtr_transfer() {
    std::unique_ptr<Test> unique_ptr1(nullptr);

    // Sanity-check value of counter
    TEST_ASSERT_EQUAL(0, Test::_instanceCount);

    // Create and destroy unique pointer in given scope
    {
        std::unique_ptr<Test> unique_ptr2(new Test);
        TEST_ASSERT_EQUAL(Test::kMagicNumber, unique_ptr2->_value);
        ...
        TEST_ASSERT(unique_ptr1.get() != unique_ptr2.get());
    }
...
static Case cases[] = {
    ...
    Case("Test ownership transfer for unique pointers", test_uniquePtr_transfer),
    Case("Test correct deletion of raw pointer", test_rawPtr_destruction)};
